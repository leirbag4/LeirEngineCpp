...
- **Window system Plan A — hit-rect expandido + borde visible (Opción 3)** (2026-09-01, ver `TODO_WINDOW_SYSTEM.md` §13): margen de contenido en **0** (edge-to-edge) con el área de resize como **anillo transparente FUERA del rect visual**. **(1) `UIElement::GetHitRect()`** virtual (default = `m_ComputedRect`); `UICanvas::HitTestRecursive` lo usa para `inside` + clip (los demás elementos no cambian). **(2) `UIWindow::GetHitRect()`** override: expande por `m_ResizeBorderSize` (6px) si resizable y no maximized → el window recibe pointer events en el anillo exterior (el content ya no roba el hit en los bordes). **(3) Content a margen 0** (`OnLayoutChrome`: offset `{0, top, cr.z, cr.w-top}`); `SetContent()` ya no copia el color del body (no hay inset que ocultar). **(4) `HitTestZone`** opera sobre `GetHitRect()` para la grilla 3×3; la title bar sigue usando el rect visual. **(5) Borde visible = Opción 3**: 4 quads vía `BuildBatch` en `UIRenderer::RenderElement` (patrón de los outlines de debug), `dynamic_cast<UIWindow*>` → `m_VisualBorderSize` (0=off, 1/2/4px) + `m_BorderColor`; sin UIElement, sin `OwnsChild`, sin teardown. **(6) Verificado en smoke**: hover del anillo exterior en el log (`HitTest: UIWindowInternal (prev hover: IntWinBody)`). **Pendiente**: verificación visual del usuario (resize desde el anillo exterior, cursor en todos lados/esquinas, borde on/off). **Nota**: los VUID de teardown en stderr (`vkDestroyPipeline/Buffer` in-use + leaks) son de la **ventana externa** (`UIWindowExternal` → `SwapchainTarget`), pre-existentes y ajenos al Plan A — pendiente de investigar.

- **Window system Fase F + Chrome profesional + Plan A documentado** (2026-08-31, ver `TODO_WINDOW_SYSTEM.md` §12-13): **(1) `UIWindowInternal`** (`engine/include/LeirEngine/UI/UIWindowInternal.h` + `src/UI/UIWindowInternal.cpp`, en el CMake del engine): ventana embedded flotando dentro del canvas (`ShowIn`/`ShowModalIn`, modal overlay, chrome propia). Editor: menú "Edit → Test Internal Window" crea una ventana interna persistente (`main.cpp` `ShowInternalTestWindow`, reutiliza con Show/Hide, delete diferido en OnUpdate). **(2) CursorStyle `ResizeNWSE`/`ResizeNESW`** (InputManager.h/.cpp → GLFW cursors). **(3) Chrome profesional**: `HitTestZone(pos)` (grilla 3×3 estilo WM_NCHITTEST, HTLEFT/HTTOPLEFT/...), `OnPointerDown/Move/Up/Exit` con cursor correcto por zona (Arrow/EW/NS/NWSE/NESW) + hover en botones min/max/close, doble-click en title bar → maximize/restore, fix clamp de posición en resize (izquierda/arriba al mínimo no desplazan la ventana), content inset 6px invisible (el window copia el color del body en `SetContent`), título centrado. **(4) Sombra de ventana**: 3 capas de `UIImage` (alpha .28/.12/.05, extras 0/4/8px sobre `m_ShadowSize` 14px) como hermanos del window en el canvas (antes que él, `SetOverlayLayer(true)`), creadas en `Activate`, destruidas en `Close`/`Hide`/dtor, reposicionadas en `OnLayoutChrome`/`BringToFront`. Toggleable: `settings.json` → `window.window_shadow` (default true, nuevo campo en `LeirSettings`) + `SetShadowEnabled`/`SetShadowSize`/`SetResizeBorderSize`/`SetVisualBorderSize`. **(5) Dos bugs de `parentOffset` corregidos** (§12.4): `ComputeLayout` SUMA `parentOffset`, y se pasaba la posición absoluta en vez de la del padre — (a) content con 2×inset a la izquierda/top y right/bottom tapados por IntWinBody (cursor resize no andaba ahí) → fix `parentOffset={cr.x,cr.y}`; (b) sombra al 2× del movimiento → fix `parentOffset={0,0}`. Regla aprendida: `parentOffset` = solo la posición del PADRE (window → `{cr.x,cr.y}`, canvas → `{0,0}`). **(6) Verificado por el usuario**: content centrado, cursor en todos los bordes/esquinas, resize sin desplazarse al mínimo, sombra sigue a la ventana, sombra on/off. **Siguiente: Plan A (§13)** — hit-rect expandido (`GetHitRect()` virtual + `HitTestRecursive` lo usa) para poder setear el margen del contenido en 0 y que el resize/cursor funcione en un anillo transparente FUERA del rect visual, + borde visible configurable (1/2/4px) dibujado encima.

- **Window system Fase A-D: SwapchainTarget + UIWindow + input multi-ventana + UIWindowExternal + AboutWindow + botón de prueba** (2026-08-31, ver `TODO_WINDOW_SYSTEM.md`): sistema de ventanas profesionales (internas/externas). **(1) SwapchainTarget** (`engine/include/LeirEngine/Rendering/SwapchainTarget.h`): surface+swapchain+views+depth+framebuffers+command buffers+sync por ventana, device/queues/pool compartidos. `VulkanDevice::CreateSwapchainTarget(window)` + `RenderBackend::CreateSwapchainTarget` virtual. **(2) Input multi-ventana**: eventos etiquetados con `void* window` (GLFWwindow*), hooks removibles por token (`EventQueue::HookId`), `InputManager::AddWindow` + per-window state (mouse pos, content scale), `UICanvas::SetInputWindow(void*)` + filtro, `GetPrimaryWindow()` + `IsPrimaryWindow()` para que el polling global solo siga la ventana principal (fix: hover de la externa ya no contamina el editor). **(3) UIWindow** (`engine/include/LeirEngine/UI/UIWindow.h`): base + chrome interno (title bar, botones min/max/close, drag, resize, modal overlay), `WindowResult` + `SetOnResult`/`SetOnAccepted`/`SetOnCanceled`, `Show/ShowModal/Close/Hide/BringToFront`, Minimize/Maximize/Restore, flags. **(4) UIWindowExternal** (`engine/include/LeirEngine/UI/UIWindowExternal.h`): ventana OS con GLFW + SwapchainTarget + UICanvas + UIRenderer propios, OS chrome, resize callback, fondo opaco (fix: swapchain garbage), refresh screen size por frame (fix: resize sigue al layout). **(5) AboutWindow** (`editor/src/UI/AboutWindow.h/.cpp`): primera ventana real, desde Help→About, con botón OK que cierra con `WindowResult::Ok`. Bug corregido: `SetFont` antes de `Show()` → null canvas crash. **(6) Botón de prueba** en la ventana de test: verifica que widgets externos reciben input independientemente del editor. Sidebar y editor: `m_Canvas->SetInputWindow(GetWindow())` + open menu, toolbar, Help → About.
