...
- [x] **Cuadraditos de plano ocultos entre sí al girar la cámara** (2026-08-22, estilo Unity): el plano rojo (pared YZ) tapaba al verde (suelo XZ) desde ciertos ángulos → el verde no se podía seleccionar. Fix: cada cuadradito de área se desplaza a lo largo de su NORMAL hacia el lado de la cámara (`side = sign(dot(camDir, n))`, offset `kPlaneOffsetPx=6px`), tanto en `Draw` como en `Pick` (el hit-test coincide con lo visual). Así los 3 planos quedan siempre visibles y seleccionables desde cualquier ángulo. El drag sigue usando el plano matemático real (normal del eje bloqueado por `startPos`); el offset es solo visual/picking.
