...
- [x] **Panel DBG + grabador de gizmo/objeto** (2026-08-22, `editor/src/UI/GizmoLogPanel.h/.cpp`): panel dockeable "DBG" con botón rojo "record gizmo log" / "stop gizmo log". Mientras graba, escribe cada frame a `<configDir>/LeirEngine/records/record_gizmo_log.txt`: cámara (pos), mouse (pos/delta/estado L/R/M down/down/released, inVp), eventos del viewport (L/R/M down/up, move), y estado del gizmo (tool T/R/S, space G/L, hover/drag handle vía `TransformGizmo::GetHoverName()/GetDragName()`) + objeto seleccionado (pos/rot/scale euler). Herramienta de diagnóstico para bugs de picking (el usuario puede decir "giré la cámara así, quise presionar el plano verde y se seleccionó el rojo" y el log muestra el hover exacto por frame). Registrado como panel dockeable y agregado a `kDebugIds` del default layout.
