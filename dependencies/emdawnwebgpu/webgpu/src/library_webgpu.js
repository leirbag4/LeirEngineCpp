...
var LibraryWebGPU = {
  ...
  wgpuDeviceCreateBindGroup: (devicePtr, descriptor) => {
    ...
    function makeEntry(entryPtr) {
      ...
      // LeirEngine patch (Fase 6, web export): support binding fixed-size
      // texture/sampler/buffer arrays (bindless table) via a chained
      // WGPUBindGroupEntryExtras (mirror of wgpu-native <webgpu/wgpu.h>).
      // Layout (wasm32): chain=0(8B) buffers=8 bufferCount=12 samplers=16
      // samplerCount=20 textureViews=24 textureViewCount=28 size=32.
      var extrasTextureViewsPtr = 0;
      var extrasTextureViewCount = 0;
      var extrasSamplersPtr = 0;
      var extrasSamplerCount = 0;
      for (var chainPtr = {{{ makeGetValue('entryPtr', 0, '*') }}}; chainPtr;
               chainPtr = {{{ makeGetValue('chainPtr', 0, '*') }}}) {
        var chainSType = {{{ makeGetValue('chainPtr', 4, 'i32') }}};
        if (chainSType === 0x00030005) { // WGPUSType_BindGroupEntryExtras
          extrasTextureViewsPtr = {{{ makeGetValue('chainPtr', 24, '*') }}};
          extrasTextureViewCount = {{{ makeGetValue('chainPtr', 28, 'i32') }}};
          extrasSamplersPtr = {{{ makeGetValue('chainPtr', 16, '*') }}};
          extrasSamplerCount = {{{ makeGetValue('chainPtr', 20, 'i32') }}};
        }
      }
...
#if ASSERTIONS
      assert((bufferPtr !== 0) + (samplerPtr !== 0) + (textureViewPtr !== 0) + (externalTexturePtr !== 0) + (extrasTextureViewCount !== 0) + (extrasSamplerCount !== 0) === 1);
#endif

      var resource;
      if (extrasTextureViewCount !== 0) {
        resource = [];
        for (var i = 0; i < extrasTextureViewCount; ++i) {
          var viewPtr = {{{ makeGetValue('extrasTextureViewsPtr', 'i * 4', '*') }}};
          resource.push(WebGPU.getJsObject(viewPtr));
        }
      } else if (extrasSamplerCount !== 0) {
        resource = [];
        for (var i = 0; i < extrasSamplerCount; ++i) {
          var samplerViewPtr = {{{ makeGetValue('extrasSamplersPtr', 'i * 4', '*') }}};
          resource.push(WebGPU.getJsObject(samplerViewPtr));
        }
      } else if (bufferPtr) {
  ...
  wgpuDeviceCreateBindGroupLayout: (devicePtr, descriptor) => {
    ...
    function makeEntry(entryPtr) {
      {{{ gpu.makeCheck('entryPtr') }}}
      // LeirEngine patch (Fase 6, web export): pass the fixed binding array
      // size through to the JS layout entry. The browser does NOT infer the
      // size from the WGSL module — omitting it yields a single-resource
      // binding that then rejects bind groups providing binding_array<T, N>.
      ...
      if (bindingArraySize) entry["bindingArraySize"] = bindingArraySize;
      WebGPU.iterateExtensions(entryPtr, {
